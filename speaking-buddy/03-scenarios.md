...
По событию `[EVENT:SCENARIO_START]` тьютор в ОДНОЙ реплике делает три шага:
объяснение (1–2 предложения) → фраза «начинаем» → первая реплика уже в роли.
Дальше он остаётся в роли до `[EVENT:SCENARIO_END]`, после которого выходит из роли и даёт
личный разбор (см. A8 в промтах — это и есть обещанный на экране «личный разбор от тьютора»).

**Язык вводной** определяется уровнем ученика, а не уровнем сценария: A1–A2 — язык объяснения,
B1+ — английский. Ниже вводные написаны на языке, который соответствует уровню самого сценария;
если тумблер «Только английский» включён, вводная всегда английская, а первая реплика в роли
не меняется никогда — она всегда на английском.

**Формат ниже:** `[emotion]` — состояние аватара; `‖` отделяет вводную от первой реплики в роли.
...
## Что передавать в промт

Поле `scenario` в блоке `[CONTEXT]` заполняется из карточки сценария:
```
scenario: { title: "Ordering Coffee", level: "A2", setting: "кофейня в час пик",
            buddy_role: "бариста", learner_role: "клиент",
            goal: "заказать напиток и что-то к нему",
            key_phrases: ["Can I get...", "for here or to take away", "medium"] }
```
Вводные из этого документа — эталон тона, а не жёсткий скрипт: тьютор строит свою вводную сам по
шаблону из A8, но должен попадать в тот же уровень, длину и настроение.

**Тяжёлые сценарии.** В `Emergency Call`, `At the Doctor's Office`, `The Card That Vanished` и
`The Report Number` есть стресс по сюжету. Правило A10 действует и внутри роли: если ученик
